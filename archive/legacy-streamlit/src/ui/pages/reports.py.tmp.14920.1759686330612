...
def render_report_builder(storage: SQLiteStorage, generator: DigestGenerator):
    ...
    if report_type == "Daily Digest":
        ...
        with col2:
            ...
            min_relevance = min_relevance_pct / 100.0

    elif report_type == "Weekly Summary":
        st.info("📅 Generates a summary of the last 7 days of activity")
        col1, col2 = st.columns(2)
        with col1:
            st.markdown("**Report covers the last 7 days**")
        with col2:
            min_relevance_pct = st.slider(
                "Minimum Relevance",
                min_value=0,
