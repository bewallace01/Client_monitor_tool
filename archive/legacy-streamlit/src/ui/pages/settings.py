...
def initialize_settings():
    ...
    if st.session_state.settings.get('database_path') == 'client_monitor.db':
        st.session_state.settings['database_path'] = 'data/client_intelligence.db'
    elif st.session_state.settings.get('database_path') == 'client_intelligence.db':
        st.session_state.settings['database_path'] = 'data/client_intelligence.db'
    ...
    if settings_file.exists():
        try:
            with open(settings_file, 'r') as f:
                ...
                # Fix incorrect database paths after loading
                if st.session_state.settings.get('database_path') == 'client_monitor.db':
                    st.session_state.settings['database_path'] = 'data/client_intelligence.db'
                elif st.session_state.settings.get('database_path') == 'client_intelligence.db':
                    st.session_state.settings['database_path'] = 'data/client_intelligence.db'
...
def render_general_settings():
    ...
    # Database Status Widget (moved from sidebar)
    ...
    try:
        ...
        with col4:
            # Show cache status
            cache_total = stats.get("total_cache_entries", 0)
            cache_valid = stats.get("valid_cache_entries", 0)
            if cache_total > 0:
                cache_ratio = f"{cache_valid}/{cache_total}"
                st.metric("Cache Valid", cache_ratio)
            else:
                st.metric("Cache", "Empty")

        st.caption(f"💡 Mode: **{st.session_state.settings.get('app_mode', 'Development')}** | 🗄️ Storage: **SQLite + DTO**")
    except Exception as e:
        st.warning("⚠️ Database not initialized or not accessible")
        st.caption("Go to Database page to initialize")
    ...
    if app_mode == "Production":
        st.warning("⚠️ Production mode is active. Mock APIs will be disabled by default.")
    ...
    debug_mode = st.toggle(
        ...
        help="Shows detailed logs, API requests/responses, database queries, and performance metrics"
    )
    st.session_state.settings['debug_mode'] = debug_mode
    st.session_state['debug_mode'] = debug_mode

    if debug_mode:
        st.info("🐛 Debug mode is active. You'll see a Debug Panel with detailed diagnostics.")
    else:
        st.caption("Enable to troubleshoot issues and monitor system performance")
    ...
    with col2:
        if st.button("⚠️ Reset Application", use_container_width=True, type="secondary"):
            st.session_state.confirm_reset = True

    if st.session_state.get('confirm_reset'):
        st.warning("⚠️ This will reset ALL application data. This cannot be undone!")
        col1, col2 = st.columns(2)
        with col1:
...
def render_api_configuration():
    ...
    if use_mock:
        st.info("📌 Using Mock APIs - No API keys required, no costs incurred")
    else:
        st.warning("⚠️ Using Real APIs - API keys required, usage costs may apply")
    ...
    if use_mock:
        st.info("💡 Disable 'Use Mock APIs' to configure real API credentials")

    # Google Custom Search
    with st.expander("🔍 Google Custom Search API", expanded=not use_mock):
        ...
        if not use_mock and st.button("🧪 Test Connection", key="test_google"):
    ...
    # NewsAPI
    with st.expander("📰 NewsAPI", expanded=not use_mock):
        ...
        if not use_mock and st.button("🧪 Test Connection", key="test_newsapi"):
            if newsapi_key:
    ...
    with st.expander("🔧 Other APIs"):
        st.info("Future integrations: Crunchbase, LinkedIn, Twitter, etc.")
        if st.button("➕ Add Custom API", disabled=True):
    ...
    st.caption("📈 Last 30 days usage (Coming soon)")
...
def render_monitoring_settings():
    ...
    st.info(f"💡 Events with relevance < {min_relevance:.0%} will be discarded")
