...
def render_clients_page():
    ...
    if 'deleting_client_id' in st.session_state:
        ...
        if client:
            # Get event count for warning
            events = storage.get_events_by_client(client.id)
            event_count = len(events)

            warning_msg = f"⚠️ Are you sure you want to delete **{client.name}**?"
            if event_count > 0:
                warning_msg += f"\n\nThis will also delete **{event_count} associated event(s)**."
            ...
            with col1:
                if st.button("✅ Yes, Delete", type="primary", use_container_width=True):
                    try:
                        ...
                        st.success(f"✅ Client '{client.name}' and {event_count} event(s) deleted")
