        ...
        if st.button("💾 Save", use_container_width=True, type="primary"):
            ...
            if not rule.name:
                ...
                if hasattr(rule, '_is_new') or not storage.get_notification_rule(rule.id):
                    ...
                    st.success(f"✅ Rule '{rule.name}' created!")
                else:
                    rule.updated_at = datetime.utcnow()
                    storage.update_notification_rule(rule)
                    st.success(f"✅ Rule '{rule.name}' updated!")
...
def render_rule_card(rule: NotificationRule, storage: SQLiteStorage):
    ...
    # Add custom styling for the container based on active status
    if rule.is_active:
        # Green border for active rules
        st.markdown(
            """
            <style>
            div[data-testid="stVerticalBlock"] > div[data-testid="stVerticalBlock"] > div[data-testid="element-container"] > div[data-testid="stMarkdown"] + div[data-testid="stHorizontalBlock"] {
        ...
        # Dashed gray border for inactive rules
        st.markdown(
            """
            <style>
            div[data-testid="stVerticalBlock"] > div[data-testid="stVerticalBlock"] > div[data-testid="element-container"] > div[data-testid="stMarkdown"] + div[data-testid="stHorizontalBlock"] {
    ...
    with st.container(border=True):
        ...
        with col4:
            if st.button("✏️", key=f"edit_{rule.id}", help="Edit"):
                st.session_state.editing_rule_id = rule.id
                st.rerun()

            if st.button("🗑️", key=f"delete_{rule.id}", help="Delete"):
...
def render_notification_history(storage: SQLiteStorage):
    ...
    for log in logs:
        ...
        type_icon = "📧" if log.notification_type == "email" else "📱" if log.notification_type == "in_app" else "🚨"
        ...
        with st.container():
            ...
            with col4:
                if st.button("👁️", key=f"view_log_{log.id}", help="View details"):
    ...
    if st.session_state.get("viewing_log_id"):
        ...
        if log:
            with st.expander("📄 Notification Content", expanded=True):
...
def render_notifications_page():
    ...
    st.markdown('<h1 class="main-header">📬 Notification Rules</h1>', unsafe_allow_html=True)
    ...
    # Tabs
    tab1, tab2 = st.tabs(["📋 Rules", "📜 History"])
